...
建议使用 [acme.sh](./acme.sh.md) 来申请免费的 Let's Encrypt 证书，具体操作请参考对应章节。
...
新建一个配置片段文件 `/etc/nginx/snippets/websocket.conf`，内容如下：
...
新建一个配置片段文件 `/etc/nginx/snippets/proxy-headers.conf`，内容如下：
